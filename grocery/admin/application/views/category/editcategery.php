...
<div class="col-md-12">
  <section class="boxs">
    ...
    <div class="boxs-body">
      <form class="form-horizontal" role="form" name="category" id="query" method="post" >
	  	
		<input type="hidden" name="cid" value="<?php echo $editcategery->categery_id ?>" >
		
        <div class="form-group">
          <label for="inputEmail3" class="col-sm-2 control-label">Categery Name</label>
          <div class="col-sm-3">
            <input type="text" name="name" id="cname" value="<?php echo $editcategery->categery_name ?>" class="form-control" id="inputEmail3" placeholder="Edit new Categery">
          </div>
        </div>
...
<script>
	function askQuery()
	{
		...
		if(name=="")
		{
			...
			document.category.cname.focus();
			return false;
		}	
		else 
		{
			document.getElementById('query').action = "<?php echo site_url()?>/categoryControl/updatedata";
