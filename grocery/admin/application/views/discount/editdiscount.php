...
<div class="col-md-12">
  <section class="boxs">
    <div class="boxs-header">
      <h3 class="custom-font hb-green"> <strong>Discount </strong>Form</h3>
    </div>
    <div class="boxs-body">
      <form class="form-horizontal" role="form" name="disc" id="query" method="post" action="<?PHP echo site_url() ?>/discountCon/updatediscount">
	  	
		<input type="hidden" name="sid" value="<?php echo $getdiscount->d_id ?>" >
		<div class="form-group">
          <label for="inputEmail3" class="col-sm-2 control-label">Product name</label>
   
		     <div class="col-sm-10">
		  	<select tabindex="3" class="chosen-select"  id="pro" style="width: 240px;" name="product">
             <?php foreach($getdata as $data) {?>
				 <option value="<?php echo $data->product_id ?>"
				 <?php echo ($data->product_id==$getdiscount->product_id)?"selected='selected'":null ?>>
        ...
        <div class="form-group">
          ...
          <div class="col-sm-3">
            <input type="text" name="discount" id="dis" value="<?php echo $getdiscount->discount ?>" class="form-control" id="inputEmail3" placeholder="Add new Discount">
          </div>
        </div>
...
<script>
	function askQuery()
	{
		var name=document.getElementById("dis").value;
		...
		if(name=="")
		{
			...
			document.disc.dis.focus();
			return false;
		}		
		else 
		{
			document.getElementById('query').action = "<?php echo site_url()?>/discountCon/updatediscount";
