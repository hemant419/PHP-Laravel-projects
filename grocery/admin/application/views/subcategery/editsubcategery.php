...
<div class="col-md-12">
  <section class="boxs">
    <div class="boxs-header">
      <h3 class="custom-font hb-green"> <strong>Category </strong>Form</h3>
    </div>
    <div class="boxs-body">
      <form class="form-horizontal" role="form" method="post" name="category" id="query">
	  	
		<input type="hidden" name="sid" value="<?php  echo $editsubcat->s_id ?>" >
		<div class="form-group">
          <label for="inputEmail3" class="col-sm-2 control-label">Categery Name</label>
   
		     <div class="col-sm-10">
		  	<select tabindex="3" class="chosen-select" style="width: 240px;" name="categery">
             <?php foreach($categery as $data) {?>
				 <option value="<?php echo $data->categery_id ?>"
				 <?php echo ($data->categery_id==$editsubcat->categery_id)?"selected='selected'":null ?>>
				 <?php echo $data->categery_name ?>
				 </option>
			<?php  } ?>
		   </select>
          </div>
        ...
        <div class="form-group">
          <label for="inputEmail3" class="col-sm-2 control-label">Categery Name</label>
          <div class="col-sm-3">
            <input type="text" name="sname" id="scname" value="<?php  echo $editsubcat->s_name ?>" class="form-control" id="inputEmail3" placeholder="Add new Categery">
          </div>
        </div>
...
<script>
	function askQuery()
	{
		...
		if(name=="")
		{
			...
			$("#nameErr").html("Please Enter SubCategory*");
			document.category.scname.focus();
			return false;
		}	
		else 
		{
			document.getElementById('query').action = "<?php echo site_url()?>/subcatControl/updatesub";
